class AMatrix():
	...
	def __init__(self, **d) -> 'AMatrix':
		...
		"""
		if 'matrix' in d:
			self.set_matrix(d['matrix'])
		elif 'empty' in d:
			self.set_matrix_empty(*d['empty'])
		elif 'random' in d:
			reflexive = d['reflexive'] if 'reflexive' in d else False
			self.set_matrix_random(*d['random'], reflexive=reflexive)
		elif 'unity' in d:
			self.set_matrix_unity(d['unity'])
		else:
			raise ValueError("No good options for arguments")
