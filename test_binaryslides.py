...
#m = graphm.MatrixBinary(boolean=matrix)
m = graphm.MatrixBinary(random=(n, n))
print('m', m)
...
#print('mc = ', len(mc), '\n', mc)
mbs = graphm.MatrixBinarySlides(mc)
print('mbs', '\n', mbs)
"""
print('------------------------')
...
"""
"""
d_from = mbs.get_paths_from(ns, shortest=True)
print('-------------------------', f'get_paths_from {ns}', sep='\n')
for i, v in d_from.items():
	print(i, '\t\t', len(v), '\n', v)

d_from = mbs.get_paths_cycle(ns, shortest=True)
print('-------------------------', f'get_paths_cycle {ns}', sep='\n')
for i, v in d_from.items():
	print(i, '\t\t', len(v), '\n', v)

""""
"""
d_from = mbs.get_paths_from_to(ns, ne, shortest=True)
print('-------------------------', f'get_paths_from_to {ns}', sep='\n')
for i, v in d_from.items():
...
while l < 3 and c < 20:
	m = graphm.MatrixBinary(random=(n, n))
	mbs = graphm.MatrixBinarySlides(m.get_closure_slides())
	dc = mbs.get_connectivity()
...
g = graphm.GraphM(binary=m.matrixM)
g.draw("files/mbs.png", nodes_style='int')
